...
if (
    selected_ticker in st.session_state["historical_data_all"]
    and not st.session_state["historical_data_all"][selected_ticker].empty
):
    ...
    st.error(
        f"No data available for the selected ETF: {ticker_label} ({selected_ticker}). "
        "Please select another ETF or check data source."
    )
...
for ticker, data in st.session_state["historical_data_all"].items():
    if not data.empty and "Close" in data.columns:
        ...
        if monthly_series.empty or len(monthly_series) < 2:  # noqa
            continue
        ...
        try:
            ...
            if pd.notna(first_price) and first_price != 0:
                ...
                st.warning(
                    f"Initial price for {etf_label} is NaN, cannot calculate cumulative returns."
                )
            else:  # Handles the case where first_price is 0
                st.warning(
                    f"Initial price for {etf_label} is zero, cannot calculate cumulative returns."
                )
        except IndexError:
            # This handles the case where monthly_series might be empty despite earlier checks
            st.warning(
                f"Could not get initial price for {etf_label} (IndexError). Skipping calculation."
            )
        except ValueError as e:
            # Catch error if .item() is called on multi-element Series or wrong type
            st.warning(
                f"Error processing initial price for {etf_label}: {e}. Skipping calculation."
            )
...
if cumulative_returns_all:
    cumulative_returns_df = pd.DataFrame(cumulative_returns_all).dropna(
        how="all"
    )  # Drop rows/cols where all are NaN
    ...
    if not cumulative_returns_df.empty:
        ...
        - Maximum Drawdown (%): The largest peak-to-trough decline during the investment period,
        indicating worst-case loss.
        - Sharpe Ratio: A risk-adjusted return measure, calculated as the annualized
        return divided by annualized volatility (assuming risk-free rate = 0).
        ...
        etf_stats = {}
        for name, ticker in etfs.items():
            data = st.session_state["historical_data_all"].get(
                ticker
            )  # Use .get for safety
            if data is None or data.empty or "Close" not in data.columns:
                st.warning(
                    f"Skipping stats calculation for {name} due to "
                    "missing or invalid base data."
                )
                continue
            ...
            if isinstance(close_col, pd.DataFrame):
                ...
                if close_col.shape[1] == 1:
                    ...
                    st.warning(
                        f"Skipping stats calculation for {name}: Unexpected structure "
                        "for 'Close' column (DataFrame)."
                    )
                    continue
            elif not isinstance(close_col, pd.Series):
                st.warning(
                    f"Skipping stats calculation for {name}: 'Close' column "
                    "is not a Series."
                )
                continue
            ...
            if len(price) < 2:  # noqa
                st.warning(
                    f"Skipping stats calculation for {name} due to insufficient "
                    "data points after cleaning (need at least 2)."
                )
                continue
            ...
            if daily_returns.empty:
                st.warning(
                    f"Skipping stats calculation for {name} as daily returns could "
                    "not be calculated (e.g., constant price)."
                )
                continue
            ...
            if start_price == 0:  # Avoid division by zero
                st.warning(
                    f"Skipping annualized return for {name} due to zero start price."
                )
                ...
                if total_period_years <= 0:
                    ...
                    annualized_return = (end_price / start_price) ** (
                        1 / total_period_years
                    ) - 1

            # Annualized Volatility
            annualized_vol = daily_returns.std() * np.sqrt(
                252
            ...
            if (
                pd.isna(annualized_vol)
                or annualized_vol == 0
                or pd.isna(annualized_return)
            ):
                sharpe_ratio = np.nan
            else:
            ...
            etf_stats[name] = {
                "Annualized Return (%)": (
                    annualized_return * 100 if pd.notna(annualized_return) else np.nan
                ),
                "Annualized Volatility (%)": (
                    annualized_vol * 100 if not np.isnan(annualized_vol) else np.nan
                ),
                "Maximum Drawdown (%)": (
                    max_drawdown * 100 if not np.isnan(max_drawdown) else np.nan
                ),
        ...
        if etf_stats:
            stats_df = pd.DataFrame(etf_stats).T.dropna(
                how="all"
...
if lump_sum > 0:
    ...
    if amount_to_invest_lump > 0 and first_price > 0:
        ...
        roi_lump = (
            ((final_lump - total_invested_lump) / total_invested_lump) * 100
            if total_invested_lump
            else 0
        )
        ...
        st.warning(
            f"Lump Sum simulation skipped: Investment amount (€{lump_sum:,.2f}) is "
            f"less than or equal to transaction cost (€{transaction_cost:,.2f})."
        )
...
if lump_sum > 0:
    ...
    for freq_label, months_in_period in investment_periods.items():
        ...
        for i, date in enumerate(simulation.index):
            ...
            months_elapsed = (date.year - simulation_start_year) * 12 + (
            ...
            if (
                months_elapsed + 1
            ) % months_in_period == 0 or i == n_total_months - 1:  # Invest in last month too if needed
        ...
        if num_investments == 0:
            st.warning(
                f"Skipping {strategy_label}: No investment periods found within the selected date range."
            )
            continue

        periodic_investment_amount = lump_sum / num_investments
        amount_to_invest_periodic = (
            periodic_investment_amount - transaction_cost
        )  # Amount available after cost

        if amount_to_invest_periodic <= 0:
            st.warning(
                f"Skipping {strategy_label}: Periodic amount (€{periodic_investment_amount:,.2f}) "
                f"is less than or equal to transaction cost (€{transaction_cost:,.2f})."
            )
            continue
        ...
        for inv_date in investment_dates:
            ...
            if price_on_inv_date > 0:
                ...
                st.warning(
                    f"Skipping investment for {strategy_label} on {inv_date.strftime('%Y-%m-%d')} due to zero price."
                )
        ...
        # ROI is calculated based on the total allocated capital
        roi = (
            ((final_value - total_allocated) / total_allocated) * 100
            if total_allocated
            else 0
        )
        final_date_str = simulation.index[-1].strftime("%Y-%m-%d")
...
if not simulation_labels:
    st.error(
        "Please enter a positive investment amount or check data availability/transaction costs."
    )
    st.stop()
...
monthly_rate = (1 + savings_rate / 100) ** (
    1 / 12
) - 1  # More accurate monthly rate from annual
...
if (
    "Monthly Investing" in simulation_labels
    and "Monthly Shares Bought" in simulation.columns
):
    ...
    num_investments_monthly = len(
        simulation
    )  # Each month is an investment period for monthly
    monthly_investment_amount = (
        lump_sum / num_investments_monthly if num_investments_monthly > 0 else 0
    )
    ...
    if monthly_investment_amount > 0:
        ...
        roi_savings = (
            ((final_savings - total_invested_savings) / total_invested_savings) * 100
            if total_invested_savings
            else 0
        )
        final_date_str = simulation.index[-1].strftime("%Y-%m-%d")
        summary_lines.append(
        ...
        st.warning(
            "Savings account simulation skipped as monthly deposit amount is zero."
        )

elif lump_sum > 0 and len(simulation) > 0:
    # Fallback if Monthly Investing didn't run but we have a lump sum and duration
    st.warning(
        "Monthly Investing strategy did not run (check transaction costs vs. monthly amount). "
        "Calculating savings based on simple monthly allocation."
    )
...
for label, series in plot_data.items():
    if label == "Lump Sum Investing":
        ...
        months_in_period = investment_periods.get(
            freq_label_part, 1
        )  # Default to 1 (monthly) if not found
        ...
        if num_investments_for_freq > 0:
            ...
            cumulative_invested = (
                (investment_occurred.cumsum() * amount_per_investment)
                .reindex(simulation.index, method="ffill")
                .fillna(0)
            )
            ...
            cumulative_returns[label] = (
                (series - cumulative_invested)
                / cumulative_invested.replace(0, np.nan)
                * 100
            )
        else:
        ...
        if monthly_investment_amount > 0:
            cumulative_invested_savings = (
                pd.Series(range(1, n_total_months + 1), index=simulation.index)
                * monthly_investment_amount
            )
            # Avoid division by zero
            cumulative_returns[label] = (
                (series - cumulative_invested_savings)
                / cumulative_invested_savings.replace(0, np.nan)
                * 100
            )
        else:
            cumulative_returns[label] = 0.0
...
st.markdown(
    f"""
This chart shows the percentage return over time for each strategy, relative to the total amount *allocated*
up to that point. Transaction costs of €{transaction_cost:,.2f} per purchase are included in the
investment strategy calculations, reducing the amount used to buy shares.

- **Lump Sum Investing:** Allocates the entire amount at the start (minus one transaction cost).
Return is relative to the initial total allocation.
- **Periodic Investing (Monthly, Quarterly, etc.):** Allocates equal amounts at regular intervals
(Dollar Cost Averaging - DCA). A transaction cost is deducted from each installment before buying
shares. Return is relative to the cumulative amount allocated so far.
- **Savings Account:** Simulates depositing the equivalent periodic amount into a savings account earning a
fixed interest rate (no transaction costs applied here). Return is relative to the cumulative principal deposited.
"""
)
